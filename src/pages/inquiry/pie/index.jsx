  ...
  render () {
    return (
      <View className='pie-chart'>
        <EChart className='chart' ref={this.refPieChart} canvasId='pie-chart' />
        {
          // this.state.detail.length==0?'':
          this.state.detail.map(item=>{
            return <View key={item.name} className='detail-item' onClick={()=>this.goDetail(item.detail)}>{item.name}</View>
          })
        }
      </View>
    )
