...
export default function index() {
      ...
      success:function(res){
        ...
        if(res.statusCode==200){
          if(res.data.length==0){
             setData([...data1,{name:'intel',kind:'text',content:'我无法理解你说的话!'}])
          }
          else if(res.data[0].custom){
              setData([...data1,{name:'intel',kind:res.data[0].custom.kind,custom:res.data[0].custom}])
          }
          else {
            res.data.map((item,index)=>{
              if(index>0) data1.push({name:'intel',kind:'text',content:res.data[index-1].text})
              return setData([...data1,{name:'intel',kind:'text',content:item.text}])
    ...
    Taro.login({
      success: function (res) {
        ...
        setData([...data, {
          name:'intel',
          kind:'none',
          content:'欢迎使用指针健康医疗服务平台,您可以在此平台咨询相关健康问题,我们为您提供2种咨询方式,您可以在下面选择',
          select:[
            '根据症状推断相关疾病',
            '咨询已知疾病相关情况'
          ]
  ...
  return <View className='intel-view'>
    <ScrollView   scrollY scrollIntoView={id} className='text' style={{height:(100-18*info.safeArea.width/info.safeArea.height)+'vh',paddingBottom:bottom}}>
     {
       data.map((item,index)=>{
           ...
           <View>
...

              {
                ...
                item.kind=='pie'?<><View><View className='content' style={{marginLeft:item.name==='intel'?'2vw':'15vw'}}>您可能出现这种疾病</View></View>
                  ...
                  <View><View className='select' onClick={other}>问问其他的</View></View></>:
                item.kind=='option'?<>{item.custom.content.map(_item=>{
                  return  <View><View className='content' key={_item} style={{marginLeft:item.name==='intel'?'2vw':'15vw'}}>{_item}</View></View>
                })}{item.custom.option.map(item=>{
                  return <View key={item.value}><View className='select' onClick={()=>intelClick(item.key,item.value)}>{item.key}</View></View>
                })}</>:
                item.kind=='none'? <><View className='content' style={{marginLeft:item.name==='intel'?'2vw':'15vw'}}>{item.content}</View>{item.select.map((_item,_index)=>{
                  return <View><View className='select' key={item.content+_index} onClick={()=>selectClick(_item,index)}>{_item}</View></View>
                    })}
                  </>
                :''
     ...
     <View className="footerButton" style={{bottom:bottom}}>
        <AtInput
            ...
            value={inputValue}
        />
        <AtButton className='button' disabled={disabled} size='small' type='primary' onClick={summit}>发送</AtButton>
      </View>
    </View>
}